...
import { FolderAlreadyLoadedException } from '../Exceptions/FolderAlreadyLoadedException'
...
export class Folder {
  ...
  createSync() {
    if (this._folderExists) {
      throw new FolderAlreadyExistException(this._name)
    }
  ...
  loadSync(options?: { withSub?: boolean; withFileContent?: boolean }) {
    ...
    if (!this._folderExists) {
      throw new FolderDoesntExistException(this._name)
    }

    if (this._files.length || this._folders.length) {
      throw new FolderAlreadyLoadedException(this._name)
    }
