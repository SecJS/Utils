...
import { FileDoesntExistException } from '../Exceptions/FileDoesntExistException'
import { FileWithoutContentException } from '../Exceptions/FileWithoutContentException'
import { FileAlreadyExistException } from '../Exceptions/FileAlreadyExistException'
import { FileAlreadyLoadedException } from '../Exceptions/FileAlreadyLoadedException'
...
export class File {
  ...
  constructor(
    ...
    if (!this._originalFileExists && !this._content) {
      throw new FileDoesntExistException(this._originalBase)
    }
  }
  ...
  createSync() {
    if (this._fileExists) {
      throw new FileDoesntExistException(this._base)
    }

    if (!this._content) {
      throw new FileWithoutContentException(this._base)
    }
  ...
  async create(): Promise<File> {
    if (this._fileExists) {
      throw new FileAlreadyExistException(this._base)
    }
  ...
  loadSync(options?: { withContent?: boolean }) {
    ...
    if (this._fileSize) {
      throw new FileAlreadyLoadedException(this._base)
    }
