...
describe('\n Folder Class', () => {
  ...
  it('should create the folder when it does not exist and throw errors if trying to create a folder that already exists and load the subFiles/subFolders information', async () => {
    try {
      ...
      expect(error.name).toBe('FolderAlreadyExistException')
    ...
    try {
      ...
      expect(error.status).toBe(500)
      expect(error.name).toBe('FolderDoesntExistException')
      expect(error.content).toBe('Folder non-existent does not exist')
    }
  ...
  it('should throw a folder already loaded exception when trying to reload/recreate the file', async () => {
    try {
      await bigFolder.create()
    } catch (error) {
      expect(error.status).toBe(500)
    ...
    try {
      ...
      expect(error.status).toBe(500)
      expect(error.name).toBe('FolderAlreadyLoadedException')
  ...
  it('should throw a folder doesnt exist exception when trying to remove the file calling remove/removeSync again', async () => {
    await nonexistentFolder.create()
